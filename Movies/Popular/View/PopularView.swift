...
class PopularView: UICollectionViewCell {
    ...
    var popularMovieData: PopularResults? {
        didSet {
            let imgUrl = "https://image.tmdb.org/t/p/w1280/" + (popularMovieData?.poster_path ?? "")
            movieImage.sd_setImage(with: URL(string: imgUrl))
            movieTitle.text = popularMovieData?.title ?? ""
            ratingNumber.text = "\(popularMovieData?.vote_average ?? 0)"
        }
    }
